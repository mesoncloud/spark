...
class EliminateInnerJoinWithAggSuite extends PlanTest {
  ...
  object Optimize extends RuleExecutor[LogicalPlan] {
    val batches =
      Batch("EliminateInnerJoinWithAgg", Once, EliminateInnerJoinWithAgg):: Nil
  }
  ...
  test("Eliminate Inner Join with Aggregate")  {
    ...
    val correctAnswer = Aggregate(Seq('d), Seq('d), Project(Seq('d),
      Join(testRelation1, testRelation2, LeftSemi, Some($"d" === $"e"), JoinHint.NONE))).analyze
